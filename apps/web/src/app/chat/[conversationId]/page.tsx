        ...
        {messages.length === 0 ? (
          <div className="flex h-full items-center justify-center">
            <div className="text-center">
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-dark-900/50 mb-4">
      ...
      <form onSubmit={handleSend} className="border-t border-dark-700/50 p-4 bg-dark-900/30 backdrop-blur-xl">
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 px-5 py-3.5 bg-dark-900/50 border border-dark-700/50 rounded-2xl text-white placeholder-slate-600 focus:outline-none focus:border-cyan-500/50 focus:ring-2 focus:ring-cyan-500/20 transition-all duration-200"
    ...
    <div className={`flex ${isSelf ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        ...
        <div className="mt-2 flex items-center gap-2">
          <p
            className={`text-xs ${isSelf ? 'text-cyan-900/70' : 'text-slate-500'}`}
          >
      ...
      {parts.map((part, i) => {
        if (part.startsWith('```')) {
          ...
          if (match) {
            ...
            return (
              <div key={i} className="overflow-x-auto rounded-2xl bg-dark-900/80 p-4">
                {lang && (
                  <div className="mb-2 text-xs text-slate-500 font-mono">{lang}</div>
                )}
